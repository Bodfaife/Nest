...
export default function ForgotPasswordScreen({
  ...
  const handleVerifyEmail = async () => {
    ...
    console.log("✅ Contact verified! Sending password reset email");
    setIsLoading(true);

    // Use supabase to send magic link / OTP with redirect to resume flow
    if (saved.email) {
      try {
        await supabase.auth.signInWithOtp({
          email: saved.email,
          options: { emailRedirectTo: `${window.location.origin}?flow=password-reset-verify` }
        });
        setIsLoading(false);
      ...
      // fallback: directly continue without email because we can't send
      setIsLoading(false);
      onVerifyEmail && onVerifyEmail(saved.email || null);
    }
  };
  ...
  return (
    <div className={`min-h-screen flex flex-col ${bgClass} p-6`}>
      ...
      <div className="flex items-center gap-4 mb-8">
        ...
        <h1 className={`text-2xl font-black ${"text-gray-900"}`}>Recover Password</h1>
      </div>
      ...
      <div className="flex-1 flex flex-col justify-start">
        ...
        {message && (
          <p className={`text-sm mb-4 ${darkMode ? 'text-green-300' : 'text-green-700'}`}>{message}</p>
        )}
