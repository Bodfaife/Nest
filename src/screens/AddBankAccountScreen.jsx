...
export default function AddBankAccountScreen({
  ...
  const [step, setStep] = useState('bank'); // 'bank' | 'account' | 'confirm'
  ...
  const handleContinue = () => {
    ...
    if (step === 'bank') {
      ...
      // Move to confirmation step (removed OTP step)
      setStep('confirm');
  ...
  return (
    <div className={`min-h-screen pb-8 flex flex-col ${bgClass} p-6 animate-in slide-in-from-right duration-300`}>
      ...
      <div className="flex gap-2 mb-8">
        <div className={`flex-1 h-1 rounded-full ${step === 'bank' || step === 'account' || step === 'confirm' ? 'bg-emerald-600' : 'bg-gray-300'}`} />
        <div className={`flex-1 h-1 rounded-full ${step === 'account' || step === 'confirm' ? 'bg-emerald-600' : 'bg-gray-300'}`} />
        <div className={`flex-1 h-1 rounded-full ${step === 'confirm' ? 'bg-emerald-600' : 'bg-gray-300'}`} />
      </div>
      ...
      <div className="flex-1 flex flex-col justify-start">
        ...
        {step === 'confirm' && (
          <>
            <div className="mb-8">
              <h2 className="text-xl font-black mb-2">Confirm Account Details</h2>
              <p className={`text-sm ${"text-gray-500"}`}>
            ...
            <div className={`p-6 rounded-2xl border-2 spacing-y-6 ${darkMode ? 'bg-gray-800 border-gray-700' : 'bg-emerald-50 border-emerald-200'}`}>
              <div className="space-y-6">
                <div>
                  <p className={`text-xs font-bold uppercase ${darkMode ? 'text-gray-400' : 'text-gray-600'}`}>Bank Name</p>
                  <p className={`text-lg font-black ${darkMode ? 'text-emerald-300' : 'text-emerald-900'}`}>{selectedBank}</p>
                </div>
                <div>
                  <p className={`text-xs font-bold uppercase ${darkMode ? 'text-gray-400' : 'text-gray-600'}`}>Account Number</p>
                  <p className={`text-lg font-black font-mono ${darkMode ? 'text-emerald-300' : 'text-emerald-900'}`}>{accountNumber}</p>
                </div>
                <div>
                  <p className={`text-xs font-bold uppercase ${darkMode ? 'text-gray-400' : 'text-gray-600'}`}>Account Holder Name</p>
                  <p className={`text-lg font-black ${darkMode ? 'text-emerald-300' : 'text-emerald-900'}`}>{accountHolder}</p>
                </div>
                <div>
                  <p className={`text-xs font-bold uppercase ${darkMode ? 'text-gray-400' : 'text-gray-600'}`}>Account Type</p>
                  <p className={`text-lg font-black capitalize ${darkMode ? 'text-emerald-300' : 'text-emerald-900'}`}>{accountType}</p>
                </div>
              </div>
            </div>

            <p className={`text-xs text-center mt-6 ${darkMode ? 'text-gray-400' : 'text-gray-600'}`}>
              An alert banner with the OTP will appear at the bottom of the app when it’s ready
            </p>
          </>
        )}
      ...
      <button
        onClick={async () => {
          // generate OTP notification when confirming bank account
          if (step === 'confirm') {
            const { generateOTP, storeOTP, notifyOTP } = await import('../lib/otpGenerator');
            const otp = generateOTP(6);
            storeOTP('addBankAccount', otp, 5);
            try {
              await notifyOTP(otp, 'addBankAccount');
            } catch (e) {
              console.error('Notification OTP failed:', e);
            }
          }
          handleContinue();
        }}
      ...
      >
        {isLoading ? 'Processing...' : (step === 'confirm' ? 'Confirm & Continue' : 'Continue')}
      </button>
    </div>
  );
}
