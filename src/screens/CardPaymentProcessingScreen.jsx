...
export default function CardPaymentProcessingScreen({
  ...
  useEffect(() => {
    ...
    const runCompletion = async () => {
      if (onPaymentComplete) {
        try {
          const result = onPaymentComplete();
          if (result && typeof result.then === 'function') await result;
        } catch (e) {
          console.error('Error during onPaymentComplete callback:', e);
        }
      }
    };

    const timer = setTimeout(async () => {
      console.log('⏳ CardPaymentProcessingScreen fired onPaymentComplete');
      await runCompletion();
      completed = true;
    }, 3000);
    ...
    const guard = setTimeout(() => {
      if (!completed) {
        console.warn('⚠️ CardPaymentProcessingScreen guard triggered after 15s, retrying onPaymentComplete');
        runCompletion();
      }
    }, 15000);
  ...
  return (
    <div className={`min-h-screen flex flex-col items-center justify-center ${bgClass} animate-in slide-in-from-right`}>
      {/* Content */}
      <div className="flex flex-col items-center justify-center space-y-8 px-6 text-center pt-12">
            {/* Animating Card */}
