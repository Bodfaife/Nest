...
const DownloadStatementScreen = ({ transactions = [], onBack, user = {}, savingsBalance: appBalance = 0 }) => {
  ...
  // Use application current balance when available; fallback to last filtered tx balance
  const currentBalance = typeof appBalance === 'number' && !Number.isNaN(appBalance)
    ? appBalance
    : (filteredTransactions.length > 0 ? (filteredTransactions[filteredTransactions.length - 1].balanceAfter || 0) : 0);

  const handleDownload = async () => {
    if (statementFormat === 'pdf') {
      const html = formatStatementAsHTML(filteredTransactions, user, currentBalance, { startDate, endDate });
      // open preview in browser tab (preview contains its own Download button)
      openStatementPreview(html);
    } else {
      const csv = formatStatementAsCSV(filteredTransactions, user, currentBalance, { startDate, endDate });
