 ...
 * - darkMode: boolean
 */
export default function PaymentProcessingScreen({ message, onComplete, darkMode = false }) {
  useEffect(() => {
    console.log('🔄 PaymentProcessingScreen mounted, setting 5s timer');
    let completed = false;
    ...
    const runCompletion = async () => {
      if (typeof onComplete === "function") {
        try {
          ...
          console.error('Error during onComplete callback:', e);
        }
      }
    };

    const timer = setTimeout(async () => {
      console.log('⏰ Processing timer fired, calling onComplete');
      await runCompletion();
      completed = true;
    }, 5000);
    ...
    const guard = setTimeout(() => {
      if (!completed) {
        console.warn('⚠️ PaymentProcessingScreen guard triggered after 15s, retrying onComplete');
        runCompletion();
      }
    }, 15000);

    return () => {
      console.log('🛑 PaymentProcessingScreen unmounted, clearing timers');
      clearTimeout(timer);
      clearTimeout(guard);
    };
  }, []);
