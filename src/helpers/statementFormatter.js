...
export const formatStatementAsCSV = (transactions, user, balance, period = {}) => {
...
`;
  ...
  // Compute period label if provided
  let periodLabel = 'All time';
  if (period.startDate && period.endDate) {
    const sd = new Date(period.startDate).toLocaleDateString('en-NG');
    const ed = new Date(period.endDate).toLocaleDateString('en-NG');
    periodLabel = `${sd} to ${ed}`;
  }

  // Summary totals using app classification: deposits = save|topup|deposit, withdrawals = withdraw
  const summary = transactions.reduce((acc, tx) => {
    if (tx.type === 'save' || tx.type === 'topup' || tx.type === 'deposit') acc.deposits += tx.amount;
    else if (tx.type === 'withdraw') acc.withdrawals += tx.amount;
    acc.count += 1;
    return acc;
  }, { deposits: 0, withdrawals: 0, count: 0 });

  const csvFooter = `\n\nStatement Period: ${periodLabel}\nTotal Transactions,${summary.count}\nTotal Deposits,₦${summary.deposits.toLocaleString('en-NG', { minimumFractionDigits: 2 })}\nTotal Withdrawals,₦${summary.withdrawals.toLocaleString('en-NG', { minimumFractionDigits: 2 })}`;
...
export const formatStatementAsHTML = (transactions, user, balance, period = {}) => {
  const timestamp = new Date().toLocaleDateString('en-NG');
  const timestamp_time = new Date().toLocaleTimeString('en-NG');

  // Determine displayed period
  ...
  if (period.startDate && period.endDate) {
    ...
    periodStart = new Date(statementDate.getFullYear(), statementDate.getMonth(), 1);
    periodEnd = new Date(statementDate.getFullYear(), statementDate.getMonth() + 1, 0);
  }

  // Calculate summary statistics using same classification as the app
  const totalDeposits = transactions
    .filter(tx => tx.type === 'save' || tx.type === 'topup' || tx.type === 'deposit')
  ...
  const csvData = formatStatementAsCSV(transactions, user, balance, period);
        ...
        <div class="content">
          ...
          <div class="summary-section">
            <div class="summary-box credits">
              ...
              <div class="summary-box-value">₦${totalDeposits.toLocaleString('en-NG', { minimumFractionDigits: 2 })}</div>
            </div>
            <div class="summary-box debits">
              <div class="summary-box-label">Total Withdrawals</div>
              <div class="summary-box-value">₦${totalWithdrawals.toLocaleString('en-NG', { minimumFractionDigits: 2 })}</div>
            </div>
          </div>
...
export const downloadPDF = async (html, filename = 'bank_statement.pdf') => {
  try {
    ...
    import('../lib/appAlert').then(({ showAppAlert }) => showAppAlert({ type: 'error', message: 'Error generating PDF: ' + error.message }));
  }
};
