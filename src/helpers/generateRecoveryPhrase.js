...
// Attempt to fetch words from an external random-word API to increase
// the pool size; fall back to a bundled list if the network call fails.

// BIP‑39 english word list (2048 words). using this gives a huge
// pool of real English terms suitable for recovery phrases.  The list was
// copied from the official spec (https://github.com/bitcoin/bips/blob/master/bip-0039/english.txt).
// Having a large static list ensures generation still works offline and
// fulfils the "billion of words/phrases in the english dictionary" request.
const FALLBACK_WORDS = [
  ...
  "craft","cram","crane","crash","crate","crave","crawl","crazy","cream","credit",
  ...
  "immune","impact","impart","improve","impulse","inch","include","income","increase","index",
  ...
  "moral","more","morning","mosquito","mother","motion","motivate","motor","mountain","mouse",
...
export async function generateRecoveryPhrase() {
  try {
    // the random-word-api returns an array of words
    const resp = await fetch('https://random-word-api.herokuapp.com/word?number=12&swear=0');
    if (resp.ok) {
    ...
    console.warn('Failed to fetch remote words for recovery phrase, using fallback', e);
  }

  // fallback local random selection from the large word list above
  const phrase = [];
  const used = new Set();
  while (phrase.length < 12) {
    const idx = Math.floor(Math.random() * FALLBACK_WORDS.length);
    if (!used.has(idx)) {
      phrase.push(FALLBACK_WORDS[idx]);
      used.add(idx);
    }
  }
  return phrase;
